                                ...
                                size_t data_size, rmt_encode_state_t* ret_state)
{
    ...
    // fall-through
    case 1: // send address
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, &scan_code->address, sizeof(uint16_t),
                                                 &session_state);
        if (session_state & RMT_ENCODING_COMPLETE)
        {
    ...
    // fall-through
    case 2: // send command
        encoded_symbols += bytes_encoder->encode(bytes_encoder, channel, &scan_code->command, sizeof(uint16_t),
                                                 &session_state);
        if (session_state & RMT_ENCODING_COMPLETE)
        {
...
esp_err_t rmt_new_ir_nec_encoder(const ir_nec_encoder_config_t* config, rmt_encoder_handle_t* ret_encoder)
{
    ...
    ESP_GOTO_ON_FALSE(config && ret_encoder, ESP_ERR_INVALID_ARG, err, TAG_T2V_MODULE_NEC_DECODER, "invalid argument");
    nec_encoder = rmt_alloc_encoder_mem(sizeof(rmt_ir_nec_encoder_t));
    ESP_GOTO_ON_FALSE(nec_encoder, ESP_ERR_NO_MEM, err, TAG_T2V_MODULE_NEC_DECODER, "no mem for ir nec encoder");
    ...
    ESP_GOTO_ON_ERROR(rmt_new_copy_encoder(&copy_encoder_config, &nec_encoder->copy_encoder), err,
                      TAG_T2V_MODULE_NEC_DECODER, "create copy encoder failed");
    ...
    ESP_GOTO_ON_ERROR(rmt_new_bytes_encoder(&bytes_encoder_config, &nec_encoder->bytes_encoder), err,
                      TAG_T2V_MODULE_NEC_DECODER, "create bytes encoder failed");
